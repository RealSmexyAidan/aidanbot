...
client.once('ready', async () => {
  ...
  const updateStats = async () => { try { const chan = await client.channels.fetch('1444216285964800093'); if (chan?.guild) await chan.setName(`👥│ ${chan.guild.memberCount} citizens`); } catch (e) { console.error(e); } };
...
client.on('messageCreate', async msg => {
  ...
  const userData = await getUserData(msg.author.id), xpGained = Math.floor(Math.random() * 11) + 15;
  let nXp = userData.xp + xpGained, nLvl = userData.level, xpNeeded = (nLvl * 50) + 50;
  if (nXp >= xpNeeded) {
    nXp -= xpNeeded; nLvl++;
    (msg.guild.channels.cache.get('1519015856837890088') || msg.channel).send({ embeds: [new EmbedBuilder().setDescription(`<@${msg.author.id}> is now **Level ${nLvl}**`).setColor('#2b2d31')] });
    try {
      const member = await msg.guild.members.fetch(msg.author.id);
      for (const m of [{ lvl: 50, id: '1505615177972846682' }, { lvl: 25, id: '1505615327873073276' }, { lvl: 10, id: '1505614729651949771' }, { lvl: 1, id: '1520015021894144130' }]) {
...
async function handleReaction(react, u, add) {
  ...
  try { const member = await react.message.guild.members.fetch(u.id); add ? await member.roles.add(rId) : await member.roles.remove(rId); } catch (e) { console.error(e); }
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'mod') {
    const sub = interaction.options.getSubcommand(), logChannelId = '1396953023426727998';
    if (sub === 'purge') {
      ...
      try {
        const del = await interaction.channel.bulkDelete(amt, true), log = interaction.guild.channels.cache.get(logChannelId);
    ...
    if (sub === 'slowmode') {
      ...
      try {
        ...
        if (log) log.send({ embeds: [new EmbedBuilder().setTitle('Channel Slowmode Updated').setColor('#2b2d31').addFields({ name: 'Channel', value: `<#${interaction.channel.id}>`, inline: true }, { name: 'Mod', value: `<@${interaction.user.id}>`, inline: true }, { name: 'Constraint Parameter', value: `\`${seconds} seconds\``, inline: true })] });
    ...
    let member; try { member = await interaction.guild.members.fetch(target.id); } catch { return interaction.reply({ content: 'Could not find that user.', ephemeral: true }); }
  ...
  if (commandName === 'level') {
    ...
    return interaction.editReply({ embeds: [new EmbedBuilder().setAuthor({ name: target.username, iconURL: target.displayAvatarURL({ dynamic: true }) }).setDescription(`**Rank:** #${rank}\n**Level:** ${uData.level}\n**XP:** ${uData.xp} / ${reqXp} XP`).setColor('#2b2d31')] });
  }

  if (commandName === 'leaderboard') {
    const [lvlRes, dapsRes] = await Promise.all([pool.query('SELECT * FROM users ORDER BY level DESC, xp DESC LIMIT 10'), pool.query('SELECT * FROM users ORDER BY daps DESC, level DESC LIMIT 10')]), medals = ['🥇', '🥈', '🥉'];
    const renderDesc = (rows, type) => rows.map((p, i) => `${i < 3 ? medals[i] : `**${i + 1}**`} <@${p.user_id}> • ${type === 'level' ? `**Level ${p.level}** • ${p.xp}/${(p.level * 50) + 50} XP` : `**${p.daps || 0} Daps**`}`).join('\n') || 'No entry items found.';
    const makeEmbed = (type) => new EmbedBuilder().setTitle(`Aidansville ${type === 'level' ? 'Level' : 'Daps'} Leaderboard`).setDescription(renderDesc(type === 'level' ? lvlRes.rows : dapsRes.rows, type)).setColor('#2b2d31').setThumbnail(interaction.guild.iconURL());
    const getRow = (active) => new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('lb_level').setLabel('Show Levels').setStyle(ButtonStyle.Primary).setDisabled(active === 'level'), new ButtonBuilder().setCustomId('lb_daps').setLabel('Show Daps').setStyle(ButtonStyle.Success).setDisabled(active === 'daps'));
    ...
    collector.on('collect', async i => { const view = i.customId === 'lb_level' ? 'level' : 'daps'; await i.update({ embeds: [makeEmbed(view)], components: [getRow(view)] }); });
  ...
  if (commandName === 'userinfo') {
    const targetUser = interaction.options.getUser('target') || interaction.user, member = await interaction.guild.members.fetch(targetUser.id).catch(() => null);
    const embed = new EmbedBuilder().setAuthor({ name: targetUser.tag, iconURL: targetUser.displayAvatarURL() }).setThumbnail(targetUser.displayAvatarURL({ dynamic: true })).setColor('#2b2d31').addFields({ name: 'Account ID', value: `\`${targetUser.id}\``, inline: true }, { name: 'Created On', value: `<t:${Math.floor(targetUser.createdTimestamp / 1000)}:F>`, inline: false });
    if (member) embed.addFields({ name: 'Joined Server', value: `<t:${Math.floor(member.joinedTimestamp / 1000)}:F>`, inline: false });
